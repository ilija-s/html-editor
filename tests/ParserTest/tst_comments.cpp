...
TEST_CASE("Parsing HTML comments", "[parsing]") {
    SECTION("Tag parser comment opening") {
        ...
        // Assert
        REQUIRE(*result1 == QVector<Token>{Token(TokenType::comment_start, 12, 4)});
    }
    ...
    SECTION("Tag parser comment closing") {
        ...
        // Assert
        REQUIRE(*result1 == QVector<Token>{Token(TokenType::comment_end, 12, 3)});
    }
    ...
    SECTION("Tag parser parses HTML even when inside a comment") {
        ...
        // Assert
        REQUIRE(*result1 == QVector<Token>{Token(TokenType::comment_start, 0, 4),
